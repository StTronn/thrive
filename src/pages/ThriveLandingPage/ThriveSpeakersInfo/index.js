...
import thriveSpeakerHeading from "../images/thriveSpeakerHeading.svg";
...
const SpeakersInfo = ({ speakersRef }) => {
  return (
    <div className="thr746Speaker " ref={speakersRef}>
      <div className="thr746SpeakerTitle">

        <Image
          src={thriveSpeakerHeading}
          alt="Meet the Speakers"
          useLazyLoad={false}
        />
      </div>
      <div className="thr746SpeakerDesc">
        Here's our exclusive list of speakers who will be appearing at THRIVE 2021.
      </div>

      <div className="thr746SpeakerGrid">
...
const SpeakerCard = ({ speaker: { info, name, title, org, img } }) => {
  return (
    <div>
      <div className="thr746SpeakerCard">
        <Image src={img} alt={"Speaker photo"} />
        <div
          className="thr746SpeakerInfo"
        >
          {info}
        </div>
      </div>
      <div className="thr746SpeakerAbout">
        <div className="thr746SpeakerName">{name}</div>
        <div className="thr746SpeakerPos">
          {title} <br /> {org}
        </div>
      </div>
    </div>
